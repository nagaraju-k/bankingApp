...
public class BankAppTestCases {
	...
	@Test
	public void test_withdraw_with_valid_openingBalance() {
		
		Account account=new Account();
			account.setAccountNo(123);
			account.setCustomer(customer);
			account.setOpeningBalance(5500);			
		Mockito.when(accountDao.findAccount(123)).thenReturn(account);
		Account account2=accountService.withdraw(account,500.0);
		Mockito.verify(accountDao).findAccount(123);
		
		assertEquals(4900.0, account2.getOpeningBalance(),100.0);			
	}
